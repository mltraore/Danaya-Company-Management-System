...
public class DriverAgent extends Personel{
	
    public void Record(DriverAgent c) throws Exception{
	...
	public int  Update(DriverAgent c) throws Exception{
		Dataccess da = Dataccess.getDataccess();
		String query = " update  driverAgent set name = '"+c.getName()+"',surname = '"+c.getSurname()+"',gender='"+c.getGender()+"',post='"+c.getPost()+"',department='"+c.getDepartement()+"',"
...
public int  Delete(int id) throws  Exception{
		...
		Dataccess da = Dataccess.getDataccess();
		String query = "delete from driverAgent where  tc = " + id;
		return  da.ExecuteUpdate(query);
	...
	public ResultSet ShowProfile(int id) throws Exception{
		...
		Dataccess da = Dataccess.getDataccess();
		String sql="select *from driverAgent where id =" + id;
		return da.ExecuteQuery(sql);
	};
	public ResultSet ShowProfiles() throws Exception{
		
		Dataccess da = Dataccess.getDataccess();
		String sql="select *from driverAgent";
		return da.ExecuteQuery(sql);
	};
	public ResultSet SearchByName(String name) throws Exception{
		
		Dataccess da = Dataccess.getDataccess();
		String search = "select *from driverAgent where name = " + name;
		return da.ExecuteQuery(search);
	};
	public Boolean Login(DriverAgent c) throws Exception{
		
		Dataccess da = Dataccess.getDataccess();
		String query = "select id from driverAgent where email = '"+c.getEmail()+"' and password= '"+c.getPassword()+"'";
